	...
	function Address(data, states, countries, validate) {
		...
		this.state = ko.pureComputed({
			read: function () {
				...
				return _.result(_.findWhere(this.states(), { name: value }), 'abbreviation');
			},
		...
		this.country = ko.pureComputed({
			read: function () {
				...
				return _.result(_.findWhere(this.countries(), { name: value }), 'code');
			},
			write: function (value) {
				this._country(value);
			},
		...
		_.mixin({
			removeConsecutive: function (array) {
				_.remove(array, function (item, idx, array) {
					return idx !== 0 && array[idx - 1] === item; //or _.isEqual for deep ofc
				});
				return array;
			}
		});
		...
		this.formatted = ko.pureComputed(function () {
			var value = this;
			var isUS = this.isUS();
			return $.trim(_.removeConsecutive(_.compact([
